          ...
          <div class="flex items-start justify-between mb-4">
            <div class="flex-1">
              <div class="flex items-center space-x-3">
                <h3 class="text-lg font-semibold text-gray-900">{{ enrollment.examFamily.name }}</h3>
...
export class EnrollmentManagementComponent implements OnInit, OnDestroy {
  ...
  private loadInitialData(): void {
    ...
    this.enrollmentService.getEnrollmentOptions().subscribe({
      ...
      error: (error) => {
        ...
        this.loading = false;
        this.showMessage('Failed to load enrollment options', 'error');
      }
    });
  }
  ...
  deleteEnrollmentConfirm(enrollment: Enrollment): void {
    if (confirm(`Are you sure you want to delete your enrollment for ${enrollment.examFamily.name}? This action cannot be undone.`)) {
